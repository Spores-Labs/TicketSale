...
import { hardCodeTime } from 'views/AnimalConcerts/Data';
import { projectData } from 'views/AnimalConcerts/Data';
...
export const AnimalConcertsBanner = () => {
  const classes = useStyles();
  const marketService = useMemo(() => getMarketService(getProjectConfigByAlias(projectAlias.animalconcerts)), []);

  const { data: receivedTimeFromServer = {} } = useQuery(
    [`${projectAlias.animalconcerts}_marketService.fetchCampaignsTime`],
  ...
  const { data: dataX = [] } = useQuery(
    [`${projectAlias.animalconcerts}_marketService.fetchProducts`],
