...
import { hardCodeTime } from 'views/Ookeenga/Data';
import { projectData } from 'views/Ookeenga/Data';
...
export const OokeengaBanner = () => {
  const classes = useStyles();
  const marketService = useMemo(() => getMarketService(getProjectConfigByAlias(projectAlias.ookeenga)), []);

  const { data: receivedTimeFromServer = {} } = useQuery(
    [`${projectAlias.ookeenga}_marketService.fetchCampaignsTime`],
  ...
  const { data: dataX = [] } = useQuery(
    [`${projectAlias.ookeenga}_marketService.fetchProducts`],
