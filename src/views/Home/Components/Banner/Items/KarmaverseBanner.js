...
import { hardCodeTime } from 'views/Karmaverse/Data';
import { projectData } from 'views/Karmaverse/Data';
...
export const KarmaverseBanner = () => {
  const classes = useStyles();
  const marketService = useMemo(() => getMarketService(getProjectConfigByAlias(projectAlias.karmaverse)), []);

  const { data: receivedTimeFromServer = {} } = useQuery(
    [`${projectAlias.karmaverse}_marketService.fetchCampaignsTime`],
  ...
  const { data: dataX = [] } = useQuery(
    [`${projectAlias.karmaverse}_marketService.fetchProducts`],
