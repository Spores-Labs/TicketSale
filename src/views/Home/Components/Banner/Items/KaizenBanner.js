...
import { hardCodeTime } from 'views/Kaizen/Data';
import { projectData } from 'views/Kaizen/Data';
...
export const KaizenBanner = () => {
  const classes = useStyles();
  const marketService = useMemo(() => getMarketService(getProjectConfigByAlias(projectAlias.kaizen)), []);

  const { data: receivedTimeFromServer = {} } = useQuery(
    [`${projectAlias.kaizen}_marketService.fetchCampaignsTime`],
  ...
  const { data: dataX = [] } = useQuery(
    [`${projectAlias.kaizen}_marketService.fetchProducts`],
