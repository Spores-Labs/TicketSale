...
import { hardCodeTime } from 'views/OokeengaINO/Data';
import { projectData } from 'views/OokeengaINO/Data';
import { projectStatuses, saleStatuses } from 'data/project-statuses';
import { isTest } from 'views/OokeengaINO/Data';
...
export const OokeengaINOBanner = () => {
  const classes = useStyles();
  const marketService = useMemo(() => getMarketService(getProjectConfigByAlias(projectAlias.ookeengaINO)), []);

  const { data: receivedTimeFromServer = {} } = useQuery(
    [`${projectAlias.ookeengaINO}_marketService.fetchCampaignsTime`],
    () => marketService.fetchCampaignsTime(),
    {
      refetchOnMount: 'always',
      refetchInterval: 60 * 1000,
      staleTime: 0,
      enabled: !isTest && (projectData.status !== projectStatuses.completed),
    },
  );
  ...
  const countdownLable = useMemo(() => {
    switch (currentRound) {
      ...
      case 2: // step 2
        return 'VIP Triple Quota ends in';
      case 3: // step 3
  ...
  const { data: dataX = [] } = useQuery(
    [`${projectAlias.ookeengaINO}_marketService.fetchProducts`],
