...
import { hardCodeTime } from 'views/Supernova/Data';
import { projectData } from 'views/Supernova/Data';
import { projectStatuses, saleStatuses } from 'data/project-statuses';

export const SupernovaBanner = () => {
  const marketService = useMemo(() => getMarketService(getProjectConfigByAlias(projectAlias.supernova)), []);

  const { data: receivedTimeFromServer = {} } = useQuery(
    [`${projectAlias.supernova}_marketService.fetchCampaignsTime`],
  ...
  const { data: dataX = [] } = useQuery(
    [`${projectAlias.supernova}_marketService.fetchProducts`],
  ...
  return (
    <div
      className='relative flex justify-center items-center pb-10 px-6 w-full'
      style={{
        display: 'flex',
        height: 800,
        background: `url(${require('assets/projects/supernova/background.png')}) no-repeat`,
    ...
    >
      ...
      >
        <Grid item xs={12} md={12} lg={7} xl={5} className='flex flex-col'>
          <div className='flex flex-col items-center xl:items-start rounded-lg py-20'>
            <img src={require('assets/projects/supernova/logo-primary.png')} className='mb-6' />
            <Typography
              variant='h1'
              align='center'
